<x-app-layout>
    <x-alert></x-alert>
    <h1 class="mb-7 font-medium text-lg text-primary">Data Siswa</h1>
    <div class="flex flex-wrap justify-between gap-4 mb-7 lg:gap-0">
        <div>
            <a href="{{ route('siswa.create') }}" class="block px-5 py-2 border border-transparent rounded-lg text-center text-sm tracking-[-0.006em] bg-dashboard font-semibold text-white transition duration-250 ease-in-out hover:bg-[#1f604f] active:bg-[#387162] disabled:opacity-25">
                Tambah Data Siswa
            </a>
        </div>
        <form action="{{ route('siswa', request()->query()) }}">
    ...
    <x-dashboard.datatable>
        <x-slot:tableColumnHeaders>
            <x-dashboard.table-column-header table-name="siswa" column-name="NIS" :sort-column="$sortColumn" :sort-direction="$sortDirection">NIS</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="siswa" column-name="nama_siswa" :sort-column="$sortColumn" :sort-direction="$sortDirection">Nama</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="siswa" column-name="jk_siswa" :sort-column="$sortColumn" :sort-direction="$sortDirection">Jenis Kelamin</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="siswa" column-name="agama_siswa" :sort-column="$sortColumn" :sort-direction="$sortDirection">Agama</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="siswa" column-name="id_kelas" :sort-column="$sortColumn" :sort-direction="$sortDirection">kelas</x-dashboard.table-column-header>
        ...
        @foreach($students as $student)
            ...
            <x-modal :id="$student->NIS">
                <x-slot:modalTitle>Hapus Data</x-slot:modalTitle>
                Apakah Anda yakin ingin menghapus data {{ $student->nama_siswa }}? Semua data terkait siswa tersebut akan permanen dihapus. Aksi ini tidak bisa dibatalkan ketika data telah terhapus.
            </x-modal>
        @endforeach

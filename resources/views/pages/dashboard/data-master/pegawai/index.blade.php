<x-app-layout>
    <x-alert></x-alert>
    <h1 class="mb-7 font-medium text-lg text-primary">Data Staff</h1>
    <div class="flex flex-wrap justify-between gap-4 mb-7 lg:gap-0">
        <div>
            <a href="{{ route('pegawai.create') }}" class="block px-5 py-2 border border-transparent rounded-lg text-center text-sm tracking-[-0.006em] bg-dashboard font-semibold text-white transition duration-250 ease-in-out hover:bg-[#1f604f] active:bg-[#387162] disabled:opacity-25">
                Tambah Data Staff
            </a>
        </div>
        <form action="{{ route('pegawai', request()->query()) }}">
    ...
    <x-dashboard.datatable>
        <x-slot:tableColumnHeaders>
            <x-dashboard.table-column-header table-name="pegawai" column-name="nama_pegawai" :sort-column="$sortColumn" :sort-direction="$sortDirection">Nama Pegawai</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="pegawai" column-name="ket_pegawai" :sort-column="$sortColumn" :sort-direction="$sortDirection">Keterangan Pegawai</x-dashboard.table-column-header>

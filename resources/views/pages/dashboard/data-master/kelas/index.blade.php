<x-app-layout>
    ...
    <div class="flex flex-wrap justify-between gap-4 mb-7 lg:gap-0">
        <div>
            <a href="{{ route('kelas.create') }}" class="flex justify-center items-center px-5 py-2 border border-transparent rounded-lg text-center text-sm tracking-[-0.006em] bg-dashboard font-semibold text-white shadow-sm transition duration-250 ease-in-out hover:bg-[#1f604f] active:bg-[#387162] disabled:opacity-25">
                Tambah Data Kelas
            </a>
        </div>
    ...
    <x-dashboard.datatable>
        <x-slot:tableColumnHeaders>
            <x-dashboard.table-column-header table-name="kelas" column-name="nama" :sort-column="$sortColumn" :sort-direction="$sortDirection">Kelas</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="kelas" column-name="wali_kelas" :sort-column="$sortColumn" :sort-direction="$sortDirection">Wali Kelas</x-dashboard.table-column-header>

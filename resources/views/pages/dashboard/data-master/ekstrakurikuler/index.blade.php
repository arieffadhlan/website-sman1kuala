<x-app-layout>
    <x-alert></x-alert>
    <h1 class="mb-7 font-medium text-lg text-primary">Data Ekstrakurikuler</h1>
    <div class="flex flex-wrap justify-between gap-4 mb-7 lg:gap-0">
        <div>
            <a href="{{ route('ekstrakurikuler.create') }}" class="flex justify-center items-center px-5 py-2 border border-transparent rounded-lg text-center text-sm tracking-[-0.006em] bg-dashboard font-semibold text-white shadow-sm transition duration-250 ease-in-out hover:bg-[#1f604f] active:bg-[#387162] disabled:opacity-25">
                Tambah Data Ekstrakurikuler
            </a>
        </div>
        <form action="{{ route('ekstrakurikuler', request()->query()) }}">
            <div class="flex justify-end h-10">
                <input type="text" name="searchExtracurricular" value="{{ $searchExtracurricular }}" class="placeholder:text-gray-400/60 form-control px-3 py-1.5 m-0 border-y border-l border-gray-400/60 rounded-l-lg bg-clip-padding text-base font-normal bg-white text-primary shadow-sm transition ease-in-out focus:ring-0 focus:!border-dashboard focus:bg-white focus:outline-none" id="datatableInput" placeholder="Cari..." aria-label="Search" aria-describedby="dataTableSearchButton">
    ...
    <x-dashboard.datatable>
        <x-slot:tableColumnHeaders>
            <x-dashboard.table-column-header table-name="ekstrakurikuler" column-name="nama_ekstrakurikuler" :sort-column="$sortColumn" :sort-direction="$sortDirection">Nama</x-dashboard.table-column-header>
            <x-dashboard.table-column-header table-name="ekstrakurikuler" column-name="ikon" :sort-column="$sortColumn" :sort-direction="$sortDirection">Ikon</x-dashboard.table-column-header>
